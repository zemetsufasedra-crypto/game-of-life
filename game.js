// ============================================================================
// EVOSPHERE - SPORE SIMULATOR v2.0 (AAA EDITION)
// ============================================================================
// Bugs fixés: pooling d'ennemis, destruction propre, accumulation mémoire,
// magnétisme balanced, écologie véritable, IA nutritive, game over fluide
...
(function() {
    ...
    // ==================== CONFIGURATION ====================
    const WORLDS = {
        1: { name: "Abysse", bg: 0x030812, color: 0x00ffcc, foodDensity: 400, enemyCount: 30 },
        2: { name: "Récif Lumineux", bg: 0x011a24, color: 0x00aaff, foodDensity: 500, enemyCount: 40 },
        3: { name: "Épave Abyssale", bg: 0x0d1117, color: 0xff6b9d, foodDensity: 450, enemyCount: 45 },
        4: { name: "Zone Thermale", bg: 0x1a0f0a, color: 0xffaa00, foodDensity: 350, enemyCount: 50, extreme: true },
        5: { name: "Utopie Terrestre", bg: 0x1a2113, color: 0x55ff22, foodDensity: 600, enemyCount: 60, terrestrial: true }
    };
    ...
    class Entity {
        ...
        eat(foodType, foodSize, amount = 1) {
            ...
            const foodEff = 1 + (this.mutations.chemosynthesis ? 0.5 : 0);
        ...
        update(delta, targetX = null, targetY = null) {
            ...
            // Mouvement IA (pour ennemis)
            if (!this.isPlayer && targetX === null) {
                // Errance aléatoire
                if (Math.random() < 0.02) {
            ...
            for (let i = 0; i < amount; i++) {
                ...
                foodPool.push({
                    ...
                    update(delta) {
                        ...
                        const color = type === 'meat' ? 0xff2a55 : 0x00ffaa;
                        this.food.clear();
            ...
            enemies.forEach((enemy, idx) => {
                ...
                if (dist < player.size + enemy.size) {
                    if (player.size > enemy.size * 1.15) {
                        const meatGain = enemy.size * 0.4;
                        player.eat('meat', enemy.size, meatGain);
                        enemyPool.release(enemy);
                        enemies.splice(idx, 1);
                        playSound(300, 0.15);
